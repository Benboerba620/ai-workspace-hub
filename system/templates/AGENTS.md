...
这个文件是 Codex 进入本工作区后的总路由。它不负责存放所有规则，只负责告诉 agent：该读哪里、写哪里、什么时候记录反馈。
...
`workspace/meta/active-context.md` 只记录最近 1-2 周仍有价值的上下文。单条尽量一行。

记录格式：

```markdown
- **YYYY-MM-DD：主题（状态）** -> 文件路径 + 一句话摘要 + 下一步锚点
```
